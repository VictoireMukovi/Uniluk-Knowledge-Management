...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
    <div class="container px-4 px-lg-20">
        ...
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation"><span class="navbar-toggler-icon">bbbb</span></button>
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <form method="POST">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0 ms-lg-10">
                <li class="nav-item"><a class="nav-link active" aria-current="page" href="../service.php">Acceuil</a></li>
                ...
                <li class="nav-item"><input type="search" class="form-control" placeholder="Recherche..." name="search"></li>               

                <li class="nav-item"><button class="btn bg-gradient-primary mt-0 w-100" class="nav-link"  name="buttonRecherche">Recherche</button></li>
...
<?php
 ...
 if(isset($_GET['id'])){
                  ...
                  <div class="card" style="width: 30rem;">
                        ...
                        <form method='POST'>
                            ...
                            <div class="col-md-12">
                                        ...
                                        <select  require="require"class="form-control" id="idfac" name="faculte" placeholder="Choisir facullté" >
                                            <?php 
                                                $fac=$getFac->fetchAll();
        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <h4>Fill d'attente pour abbrobation d'ouvrage</h4>
                </div>
                <div class="card-body">
                    <table class="table table-bordered table-striped">
                        ...
                        <?php
                            ...
                            $selectF=$connexion->query("SELECT * FROM tfichiertemp WHERE statut=0 ");
                        
                           while($item=$selectF->fetchAll()) {
                            ...
                            if($item >0){
                                ...
                                ?>
                                    ...
                                    <td><?=$item['id_classeur'];?></td>
                                    <td><?=$item['corps'];?></td>
                                    <td><a href="forms/fichier<?=$item['corps']; ?>" class="btn btn-primary">Ouvrir</a></td>
                                    <td><a href="forms/fichier<?=$item['corps']; ?>" class="btn btn-primary">Rejeter</a></td>
                                    <td><a href="forms/fichier<?=$item['corps']; ?>" class="btn btn-primary">Approuver</a></td>
    ...
    $getAllFile=$connexion->query("SELECT * FROM tfichier WHERE id_classeur=$id ORDER BY id DESC");
                ...
                <div class="container px-4 px-lg-5 mt-5">
                  <div class="card" style="width: 24rem;">
                        ...
                        <p class="card-text">
                         ...
                         <form method="GET">
                               <a href="liens.php?idf=<?=$file['id'];?>" class="btn btn-outline-dark stretched-link">Ouvrir le fichier</a>
                               <a href="liens.php?idf=<?=$file['id'];?>" class="btn btn-outline-dark stretched-link">Telecharger</a>
                        </form>
                        <h5 class="card-title"><i class="bi bi-file-earmark-word-fill"><?php 
...
//include('connexion.php');
        if(isset($_POST['btnEnregClsr'])){
           ...
           if ($sizefile<41943040 ) {
            ...
            $InsertIntoActiviter= $connexion->prepare("INSERT INTO tactivite(description_activite,procedure_activite,id_decanat,fichier) VALUES(?,?,?,?)");
              ...
              // $annee=$_POST['annee'];
               ...
               // $idtypefichier=$_POST['typefichier'];
                ...
                ?>
                    ...
                    <script>
                        swal("Fichier directement publier avec success","Clique pour continuer","success");
...
<?php
               ...
               //$getAllFile=$connexion->query("SELECT * FROM tfichier WHERE id_classeur=$id ORDER BY id DESC");
               //Vérification si une recherche à été rentrer par l'Utilisateur
               if (isset($_POST['buttonRecherche']) ) {
                  // $userSearch=$_GET['search'];
                  
                  // echo($userSearch);
                  $i=$_SESSION['faculte'];
                  
                  $selectFact=$connexion->prepare("SELECT * FROM tclasseur WHERE id_faculte=?");
                  $selectFact->execute(array($i));

                 while($f=$selectFact->fetch()){}
                  $fi=$f['id_classeur'];
                  
                  $val=$_POST['search'];
                
                   $getAllFile=$connexion->prepare("SELECT * FROM tfichier WHERE description_fichier LIKE ? ");
                   $getAllFile->execute(array('%'.$val.'%'));
                

                   if($getAllFile->rowCount()>0){
           
                    while($file=$getAllFile->fetch()){
                      $idC = $file['id_classeur'];
                      $selectClasseur=$connexion->prepare("SELECT * FROM tclasseur WHERE id_classeur=?");
                      $selectClasseur->execute(array($idC));
                      $c=$selectClasseur->fetch();
                      if(($c['id_faculte'])==($_SESSION['faculte'])){
                            ...
                            <div class="container px-4 px-lg-5 mt-5">
                              <div class="card" style="width: 24rem;">
                                    ...
                                    <h5 class="card-title"><i class="bi bi-file-earmark-word-fill"><?php 
                                     ...
                                     echo($file['description_fichier'].($val)) ; ?>  </i></h5>
                                    <p class="card-text">
                           
                           </p>
                                     <form method="GET">
                                           <a blank href="liens.php?idf=<?=$file['id'];?>" class="btn btn-outline-dark stretched-link">Ouvrir le fichier</a>
                                           <a href="liens.php?idf=<?=$file['id'];?>" class="btn btn-outline-dark stretched-link">Telecharger</a>
                                    </form>
                                    <h5 class="card-title"><i class="bi bi-file-earmark-word-fill"><?php 
                                    $idU=$file['id_utilisateur'];
                   ...
                   <?php 
                            ...
                            ?>
                                 <script rel="stylesheet" src="../cdnjs/sweetalert.min.js"></script>
                                <script>
                                            swal("Mot clé de recherche intouvable","Clique pour continuer","error");
